...
const testimonials = TESTIMONIALS.filter((t) => t.tier === 2);
...
export function ForgeProgramClient() {
    ...
    <main className="bg-white text-[#0f1f20]">
      ...
      <section className="py-16 md:py-28 bg-white">
        <div className="max-w-5xl mx-auto px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-2 gap-6 md:gap-8">
            {testimonials.map((t) => (
              <div key={t.id} className="border border-[#d0e8ea] rounded-2xl p-6 md:p-8 flex flex-col gap-5">
                <p className="text-[#3d5a5c] text-base md:text-lg italic leading-relaxed font-serif">
                  "{t.quote}"
                </p>
                <div>
                  <p className="text-[#0f1f20] font-semibold text-sm md:text-base">{t.author}</p>
                  {t.role && <p className="text-[#7a9ea1] text-sm">{t.role}</p>}
                </div>
              </div>
            ))}
          </div>
